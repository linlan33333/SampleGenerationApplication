<template>
  <q-card flat class="full-width">
    <q-card-section>
      <div class="text-container flex wrap justify-start q-gutter-sm">
        <q-item v-for="(fileInfo, index) in filesInfo" :key="index"
                class="text-item column">
          <q-item-section class="full-width">
            <q-img src="~assets/text.jpg"
              ...
              <q-checkbox v-show="multiSelect" class="all-pointer-events"
                          v-model="selectedTexts" :val="fileInfo" color="orange" size="sm" />
            </q-img>
          </q-item-section>
    ...
    onMounted(() => {
      ...
      bus.on('batchSaveFiles', params => {
        if (selectedTexts.value.length !== 0) {
          // 覆盖原文件
          for (let file of selectedTexts.value) {
