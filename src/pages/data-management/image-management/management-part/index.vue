<template>
  <div class="column full-width flex flex-center full-height">
    <div class="full-width">
      <tool-bar @newFolder="getNewFolderName" @searchImageName="getSearchImageName" @importFolderPath="getImportFolderPath" />
    </div>
    ...
    <q-scroll-area :thumb-style="thumbStyle" class="scroll-area-height full-width">
      <folder-list v-if="show" @selectedImgUrl="getSelectedImgUrl" :searchImageName="searchImageName"
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      searchImageName,   // toolbar上的搜索框中输入的图片名称，需要交给FolderList组件进行响应式搜索
      importFolderPath,   // 数据导入功能所传入的文件夹路径
      afterModFolder,
      show,

      getSelectedImgUrl(imgUrl, imgFolder) {
        emit("imgPartSelectedImg", imgUrl, imgFolder);
      },

      // 拿到toolbar中输入的待搜索的图片名称
      getSearchImageName(data) {
        searchImageName.value = data;
      },
